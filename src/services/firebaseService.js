...
// Test Groups Service
export const testGroupsService = {
  async getTestGroups() {
    try {
      const q = query(collection(db, "testGroups"), orderBy("createdAt", "asc"));
      const querySnapshot = await getDocs(q);
  ...
  async addTestGroup(data) {
    try {
      ...
      const docRef = await addDoc(collection(db, "testGroups"), payload);
      return { id: docRef.id, ...payload };
    } catch (error) {
  ...
  async updateTestGroup(id, data) {
    try {
      const ref = doc(db, "testGroups", String(id));
      const update = {
        name: String(data?.name || "").trim(),
        description: String(data?.description || "").trim(),
        updatedAt: new Date(),
  ...
  async deleteTestGroup(id) {
    try {
      await deleteDoc(doc(db, "testGroups", String(id)));
      return true;
    } catch (error) {
...
export const testsService = {
  async getTests() {
    try {
      const q = query(collection(db, "tests"), orderBy("createdAt", "asc"));
      const querySnapshot = await getDocs(q);
      return querySnapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
    } catch (error) {
  ...
  async addTest(data) {
    try {
      ...
      const payload = {
        ...
        testGroupId: String(data?.testGroupId || "").trim(),
        testGroupName: String(data?.testGroupName || "").trim(),
        createdAt: now,
        updatedAt: now,
      };
  ...
  async updateTest(id, data) {
    try {
      const ref = doc(db, "tests", String(id));
      const update = {
        title: String(data?.title || "").trim(),
        link: String(data?.link || "").trim(),
        testGroupId: String(data?.testGroupId || "").trim(),
  ...
  async deleteTest(id) {
    try {
      await deleteDoc(doc(db, "tests", String(id)));
      return true;
    } catch (error) {
  ...
  async bulkAssignGroup(testIds, testGroupId, testGroupName) {
    try {
      await Promise.all(
        testIds.map((id) => {
          const ref = doc(db, "tests", String(id));
          return updateDoc(ref, {
            ...
            testGroupName: String(testGroupName || ""),
            updatedAt: new Date(),
          });
        }),
      );
